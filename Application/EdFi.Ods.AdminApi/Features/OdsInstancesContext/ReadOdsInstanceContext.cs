// SPDX-License-Identifier: Apache-2.0
// Licensed to the Ed-Fi Alliance under one or more agreements.
// The Ed-Fi Alliance licenses this file to you under the Apache License, Version 2.0.
// See the LICENSE and NOTICES files in the project root for more information.

using AutoMapper;
using EdFi.Ods.AdminApi.Infrastructure;
using EdFi.Ods.AdminApi.Infrastructure.Database.Queries;

namespace EdFi.Ods.AdminApi.Features.OdsInstancesContext;

public class ReadOdsInstanceContext : IFeature
{
    public void MapEndpoints(IEndpointRouteBuilder endpoints)
    {
        AdminApiEndpointBuilder.MapGet(endpoints, "/odsInstancesContext", GetOdsInstanceContexts)
            .WithDefaultDescription()
            .WithRouteOptions(b => b.WithResponse<OdsInstanceContextModel[]>(200))
            .BuildForVersions(AdminApiVersions.V2);

        AdminApiEndpointBuilder.MapGet(endpoints, "/odsInstancesContext/{id}", GetOdsInstanceContext)
            .WithDefaultDescription()
            .WithRouteOptions(b => b.WithResponse<OdsInstanceContextModel>(200))
            .BuildForVersions(AdminApiVersions.V2);
    }

    internal Task<IResult> GetOdsInstanceContexts(IGetOdsInstanceContextsQuery getOdsInstanceContextsQuery, IMapper mapper, int offset, int limit)
    {
        var odsInstanceContextList = mapper.Map<List<OdsInstanceContextModel>>(getOdsInstanceContextsQuery.Execute(offset, limit));
        return Task.FromResult(Results.Ok(odsInstanceContextList));
    }

    internal Task<IResult> GetOdsInstanceContext(IGetOdsInstanceContextByIdQuery getOdsInstanceContextByIdQuery, IMapper mapper, int id)
    {
        var odsInstanceContext = getOdsInstanceContextByIdQuery.Execute(id);
        if (odsInstanceContext == null)
        {
            throw new NotFoundException<int>("odsInstanceContext", id);
        }
        var model = mapper.Map<OdsInstanceContextModel>(odsInstanceContext);
        return Task.FromResult(Results.Ok(model));
    }
}
