...
public class ReadTenants : IFeature
{
    ...
    public static async Task<IResult> GetTenantsAsync([FromServices] IAdminConsoleTenantsService adminConsoleTenantsService, IMemoryCache memoryCache, IOptions<AppSettings> options)
    {
    ...
    public static async Task<IResult> GetTenantsByTenantIdAsync([FromServices] IAdminConsoleTenantsService adminConsoleTenantsService,
        IMemoryCache memoryCache, string tenantName, IOptions<AppSettings> options)
    {
        var _databaseEngine = options.Value.DatabaseEngine ?? throw new NotFoundException<string>("AppSettings", "DatabaseEngine");

        var tenant = await adminConsoleTenantsService.GetTenantByTenantIdAsync(tenantName);
        if (tenant == null)
