...
public class ReadTenantsTest
{
    ...
    {
        var adminConsoleTenantsService = A.Fake<IAdminConsoleTenantsService>();
        ...
        A.CallTo(() => adminConsoleTenantsService.GetTenantByTenantIdAsync(tenantName)).Returns(tenant);

        var result = await ReadTenants.GetTenantsByTenantIdAsync(adminConsoleTenantsService, memoryCache, tenantName, options);
    ...
    public async Task GetTenantsByTenantIdAsync_ReturnsNotFound_WhenTenantDoesNotExist()
    {
        ...
        A.CallTo(() => options.Value).Returns(new AppSettings { DatabaseEngine = "Postgres" });
        A.CallTo(() => adminConsoleTenantsService.GetTenantByTenantIdAsync(tenantName)).Returns((TenantModel)null);
    ...
    public async Task GetTenantsAsync_ReturnsOk_WithTenantList()
    {
        ...
        A.CallTo(() => options.Value).Returns(new AppSettings { DatabaseEngine = "Postgres" });
        A.CallTo(() => adminConsoleTenantsService.GetTenantsAsync(true)).Returns(Task.FromResult(tenants));

        var result = await ReadTenants.GetTenantsAsync(adminConsoleTenantsService, memoryCache, options);
    ...
    public void GetTenantsByTenantIdAsync_ThrowsNotFoundException_WhenDatabaseEngineIsNull()
    {
        ...
        {
            await ReadTenants.GetTenantsByTenantIdAsync(adminConsoleTenantsService, memoryCache, tenantName, options);
        });
    }
    ...
    public void GetTenantsAsync_ThrowsNotFoundException_WhenDatabaseEngineIsNull()
    {
        ...
        Should.ThrowAsync<NotFoundException<string>>(async () =>
        {
            await ReadTenants.GetTenantsAsync(adminConsoleTenantsService, memoryCache, options);
        });
    }
}
