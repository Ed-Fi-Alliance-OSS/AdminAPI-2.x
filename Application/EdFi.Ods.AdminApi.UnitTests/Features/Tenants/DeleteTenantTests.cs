...
{
    private IAdminConsoleTenantsService _tenantsService = null!;
    ...
    public void SetUp()
    {
        _tenantsService = A.Fake<IAdminConsoleTenantsService>();
    ...
    public async Task Handle_ShouldDeleteTenantAndReturnOk_WhenTenantExists()
    {
        ...
        var scopedTenantsService = A.Fake<IAdminConsoleTenantsService>();

        A.CallTo(() => _serviceScopeFactory.CreateScope()).Returns(scope);
        A.CallTo(() => scope.ServiceProvider).Returns(scopedServiceProvider);
        A.CallTo(() => scopedServiceProvider.GetService(typeof(IAdminConsoleTenantsService))).Returns(scopedTenantsService);
