...
public class GetOdsInstanceContextsQuery : IGetOdsInstanceContextsQuery
{
    ...
    public GetOdsInstanceContextsQuery(IUsersContext usersContext, IOptions<AppSettings> options)
    {
        ...
        {
            { SortingConstants.OdsInstanceContextKeyColumn, x => isSQLServerEngine ? EF.Functions.Collate(x.ContextKey, DatabaseEngineEnum.SqlServerCollation) : x.ContextKey },
            { SortingConstants.OdsInstanceContextValueColumn, x => isSQLServerEngine ? EF.Functions.Collate(x.ContextValue, DatabaseEngineEnum.SqlServerCollation) : x.ContextValue },
            { SortingConstants.DefaultIdColumn, x => x.OdsInstanceContextId }
        };
    }
